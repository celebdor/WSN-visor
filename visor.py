...
class wsnVisor:
    """ wsnVisor is a module that takes a testbed output file, turns each entry into a networkx representation and generates a picture of it through the display module. """
    def __init__(self):
    ...
    def highwayEdges(self, n, pS, pT, cS, cT):
        """ Recursively adds all the edges from port to leader of a certain highway.

        Keyword arguments:
        n --- The node from which to start adding edges.
        pS --- The port source id.
        pT --- The port target id.
        cS --- The cluster source id.
        cT --- The cluster target id. """
        g = self.graph
        if not n.isLeader():
            aux = wsnNode( n.parent, n.sid, n.sid)
            if g.has_node(aux):
...
class wsnNode:
    """ This module models a wireless sensor netowork node with its id, cluster id and parent node. """
    def __init__(self, i, s, p):
        ...
        p --- id of the wsnNode's parent. """
        self.id = i
        self.sid = s
        self.parent = p

    def __hash__(self):
        """ Returns the hash value of a wsnNode. """
        return self.id;
    ...
    def __str__(self):
        """ Returns a wsnNodes representation with the following format:
